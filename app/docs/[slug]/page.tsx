...
import { isDocSlug, neighbors, PAGE_META, PAGES_ORDER, type DocSlug } from "../_data/routes"
...
export function generateStaticParams() {
  return PAGES_ORDER.filter((slug) => slug !== "introduction").map((slug) => ({ slug }))
}
...
export default async function DocsSlugPage({ params }: PageParams) {
  const { slug } = await params
  if (!isDocSlug(slug) || slug === "introduction") notFound()
